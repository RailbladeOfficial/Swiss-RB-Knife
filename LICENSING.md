# Licensing and Terms

## Swiss RB Knife

**Copyright © 2026 Railblade**

Swiss RB Knife is free and open-source software, released under the **GNU Affero General Public License, Version 3 (AGPL-3.0)**.

The full license text is available in the [LICENSE](LICENSE) file included with this software.

---

## What This License Means (Plain Language)

### You Are Free To:

- **Use** this software for any purpose, personal or commercial.
- **Study** how it works and learn from the source code.
- **Modify** the software to suit your own needs.
- **Distribute** copies of the original or your modified version.
- **Sell services** around this software: support, consulting, hosted versions, etc.
- **Advertise** modifications or forks you've made.

### Under These Conditions:

- **Keep it open.** Any modifications or forks you distribute must also be released under AGPL-3.0. You cannot make the code proprietary or sell it as closed-source software.
- **Network use counts as distribution.** If you run a modified version as a networked service (e.g., a hosted web app), you must make the modified source code publicly available. This is what sets AGPL apart from standard GPL.
- **Preserve attribution.** All distributions must retain the original copyright notice and credit to Railblade as the original author.
- **State your changes.** If you modify the software and distribute it, you must clearly indicate what changes were made.

### You May Not:

- Remove or obscure the copyright notices.
- Re-license this software under a different license.
- Distribute this software, or any derivative of it, without making the full source code available under AGPL-3.0.
- Claim ownership of the original work.

---

## Third-Party Software

Swiss RB Knife depends on various open-source libraries and packages. Each is used in accordance with its own license. A full list of dependencies and their respective licenses is available in [THIRD_PARTY_LICENSES.md](THIRD_PARTY_LICENSES.md).

---

## Attributions

Credits for sound effects, tools, and technologies used in this project are listed in [ATTRIBUTION.md](ATTRIBUTION.md).

---

## Contact

Swiss RB Knife is a personal project maintained by **Railblade**. There is no official support channel, but the source code is available for review, contribution, or forking in accordance with the terms above. You are free to open an issue here in GitHub, or you can also reach out on Discord (@railblade).

---

_This document is a human-readable summary of the AGPL-3.0 license and does not replace or supersede the full license text. In the event of any conflict, the full [LICENSE](LICENSE) text governs._