/* =============================================================================
   ...
   Tabs / commands:
     ...
     Compress — scale a single image by percentage
                (compress_image)
     Resize   — batch-resize a folder to a target dimension / canvas size,
                running on a dedicated thread with live progress events
                (get_resize_folder_stats, resize_images, cancel_resize)
     Utility  — show_in_explorer (shared across tabs)

   The resize job is spawned onto its own OS thread so the Tauri IPC pool stays
   free for the duration of a long batch. Progress is communicated via
   "resize-progress" and "resize-complete" Tauri events.
============================================================================= */
...
/// Parse a CSS hex color string (#RRGGBB or #RGB) into [r, g, b, 255].
...
// =============================================================================
//  get_resize_folder_stats
// =============================================================================

#[derive(serde::Serialize)]
pub struct ResizeFolderStats {
    count: u32,
    min_w: u32,
    max_w: u32,
    min_h: u32,
...
/// Scans a folder for supported images and returns dimension statistics.
#[tauri::command]
pub fn get_resize_folder_stats(folder: String) -> Result<ResizeFolderStats, String> {
    ...
    if count == 0 {
        return Ok(ResizeFolderStats { count: 0, min_w: 0, max_w: 0, min_h: 0, max_h: 0 });
    }

    Ok(ResizeFolderStats { count, min_w, max_w, min_h, max_h })
}
...
pub fn resize_images(
    ...
    std::thread::spawn(move || {
        resize_images_thread(
            app, source_folder, output_folder,
...
fn resize_images_thread(
    ...
    let out_dir: PathBuf = if let Some(f) = output_folder {
        ...
        PathBuf::from(&source_folder).join(format!("resized-{timestamp}"))
    };
    ...
    // ── Collect image paths first so we know the total ───────────────────────
    let entries = match std::fs::read_dir(&source_folder) {
        Ok(e)  => e,
        Err(e) => {
            let _ = app.emit("resize-complete", ResizeCompleteEvent {
                success: false,
                message: format!("Cannot read source folder: {e}"),
                output_folder: out_dir.to_string_lossy().to_string(),
                count: 0,
            });
            return;
    ...
    if total == 0 {
        let _ = app.emit("resize-complete", ResizeCompleteEvent {
            success: false,
            message: "No supported images found in the source folder.".to_string(),
            output_folder: out_dir.to_string_lossy().to_string(),
            count: 0,
        });
        return;
    ...
    // If the user pointed the output folder AT the source folder, an output
    // name can collide with an ORIGINAL and overwrite it — actual data loss,
    // not just a lost output. Pre-seeding the set with the source filenames
    // makes those collisions take the counter-suffix path instead.
    let same_dir = {
        let a = out_dir.to_string_lossy().replace('/', "\\").trim_end_matches('\\').to_lowercase();
        let b = source_folder.replace('/', "\\").trim_end_matches('\\').to_lowercase();
        a == b
    };
    if same_dir {
    ...
    for src_path in &image_paths {
        ...
        // Emit progress before processing so the UI shows the current file
        ...
        let mut suffix = 2u32;
        while used_names.contains(&out_name.to_lowercase()) {
        ...
        if let Err(e) = final_img.save_with_format(&out_path, out_fmt) {
            let _ = app.emit("resize-complete", ResizeCompleteEvent {
                success: false,
                message: format!("Failed to save {}: {e}", out_path.display()),
                output_folder: out_dir.to_string_lossy().to_string(),
                count,
            });
            return;
