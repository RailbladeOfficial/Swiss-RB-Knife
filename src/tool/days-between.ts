...
export function initDaysBetween(): void {
  ...
  // A date input is not a text field, so it has no native right-click menu to
  // preserve (see isTextEntry in shell.ts) and the gesture is free here.
  // Copy is the one that matters: a <input type="date"> holds its value as a
  // picker, not as selectable text, so there is otherwise no way to get the
  // date out of this tool and into anything else.
      ...
      },
      {
        ...
        onClick: () => {
          void navigator.clipboard
            ...
            .then(() => flash("Date copied", "success"))
            .catch(() => flash("Couldn't reach the clipboard", "error"));
        },
      },
      { label: "Swap Start and End", onClick: swapDates },
      {
        label: "Reset Both Dates",
        onClick: () => {
          resetDates();
          flash("Tool reset", "success");
        },
