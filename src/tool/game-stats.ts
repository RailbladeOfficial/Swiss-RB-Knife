...
function buildGameRow(game: GameInstance): HTMLElement {
  ...
  // Delete lives inside the game's own detail view, so removing a game from
  // the list means opening it, deleting, and being returned to the list. This
  // is the way to do it from where you are actually looking.
  attachMenu(row, () => [
    { label: "Open Game", onClick: open },
    {
      label: "Copy Game Summary",
      onClick: () => {
    ...
    },
    {
      label: "Delete Game",
      danger: true,
      onClick: () => openGameDelete(game.id, gameLabel(game)),
    },
  ]);
 ...
 *  would leave a chart drawn in whatever color was last set. Themes state
 *  colors as hex OR rgb()/rgba()/hsl() (e.g. cake's translucent muted text),
...
export function initGameStats(): void {
  ...
  // Chart colors are read from CSS custom properties at draw time (see
