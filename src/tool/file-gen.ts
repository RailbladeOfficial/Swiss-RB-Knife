...
function renderBatchList(): void {
  ...
  batchEntries.forEach((entry, idx) => {
    ...
    // Duplicate and the two moves are all new here: the card's only button is
    // Remove, so building a second batch that differs by one field currently
    // means retyping the other four. The card's inputs keep their own native
    // Cut/Copy/Paste, which attachMenu leaves alone.
    attachMenu(card, () => [
      {
        label: "Duplicate Entry",
        onClick: () => {
          batchEntries.splice(idx + 1, 0, { ...batchEntries[idx] });
          renderBatchList();
        },
      },
      {
        ...
        disabled: idx === 0,
        onClick: () => {
          const [moved] = batchEntries.splice(idx, 1);
          batchEntries.splice(idx - 1, 0, moved);
          renderBatchList();
        },
      },
      {
        label: "Move Down",
        disabled: idx >= batchEntries.length - 1,
        onClick: () => {
          const [moved] = batchEntries.splice(idx, 1);
          batchEntries.splice(idx + 1, 0, moved);
          renderBatchList();
      ...
      { label: "Pick Extension…", onClick: () => openExtModal(extEl) },
      {
        label: "Remove Entry",
        danger: true,
        // The last remaining entry's Remove button is disabled, because a
        // batch with no entries has nothing to generate.
