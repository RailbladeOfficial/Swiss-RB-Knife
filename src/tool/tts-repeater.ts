...
interface TtsDisplaySettings {
   ...
   *  two exact colors and is flat by definition. */
  glow: boolean;
...
/** Same job as normalizeConfig(), for the display settings. A hex color that
 *  isn't a hex color would silently blank an <input type="color">, so the
 ...
 *  original's inline backgroundColor, so themes stay in charge of the colors. */
 ...
 *
 *  Follow-theme mode sets NO inline colors at all. It hands off entirely to
 ...
 *  bolted on top. Custom mode is the opposite by design: two exact colors,
 *  flat, no effects. */
function applyDisplayAppearance(root: HTMLElement, textEl: HTMLElement): void {
  ...
  // Lets the active theme paint its full-window ambience onto the card, which
  // it otherwise cannot: the overlay covers the window those layers live on.
  // Same call the Countdown Timer's overlay makes; see applyDisplaySurface().
  ...
  textEl.classList.toggle("tts-display-themeglow", themed && display.glow && themeHasTitleGlow());
...
/** Derived Setup UI: the custom-color row's visibility, the size read-out,
...
function renderPresetList(): void {
  ...
  presets.forEach((preset) => {
    ...
    `;
    const deleteBtn = row.querySelector<HTMLButtonElement>(".tts-preset-delete")!;
    ...
    attachMenu(row, () => [
      { label: "Load Preset", onClick: () => loadPreset(preset.id) },
      {
        label: "Copy Message",
        onClick: () => {
          void navigator.clipboard
            .writeText(preset.message)
            .then(() => flash("Message copied.", "success"))
            .catch(() => flash("Couldn't reach the clipboard.", "error"));
        },
      },
      { label: "Delete Preset", danger: true, onClick: () => deleteBtn.click() },
    ]);
