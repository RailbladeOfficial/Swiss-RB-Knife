...
function renderSourceList(): void {
  ...
  config.sources.forEach((src, i) => {
    ...
    // "Edit path" is double-click-only, and the only thing that says so is a
    // tooltip. Naming it in a menu is the point of this one.
    attachMenu(row, () => [
      {
        label: "Edit Path",
        disabled: backupRunning,
        onClick: () => makePathEditable(label, "sources", i),
      },
      {
        label: "Browse to Replace…",
        disabled: backupRunning,
        onClick: () => browseBtn.click(),
      },
      { label: "Copy Path", onClick: () => void copyPath(src) },
      {
        label: "Remove Source",
        danger: true,
        disabled: backupRunning,
    ...
    attachMenu(row, () => [
      {
        label: "Edit Path",
        disabled: backupRunning,
        onClick: () => makePathEditable(label, "destinations", i),
      },
      {
        label: "Browse to Replace…",
        disabled: backupRunning,
      ...
      { label: "Copy Path", onClick: () => void copyPath(dest) },
      {
...
/** Copies one path to the clipboard, with the same success/failure toast the
 *  rest of the app uses for a clipboard write. */
async function copyPath(path: string): Promise<void> {
  try {
    await navigator.clipboard.writeText(path);
    flash("Path copied", "success");
  } catch {
...
function renderPresetList(): void {
  ...
  for (const preset of presets) {
    ...
    // Load and Delete live on the editor panel to the side, which only fills
    // in once a preset is selected. From the list itself they are two clicks
    // and a look; here they are one.
    attachMenu(item, () => [
      { label: "Select", onClick: () => selectPreset(preset.id) },
      {
        label: "Load into Main",
        onClick: () => {
          selectPreset(preset.id);
          document.getElementById("ab-preset-load-btn")!.click();
        },
      },
      {
        label: "Delete Preset",
        ...
        onClick: () => {
          selectPreset(preset.id);
          document.getElementById("ab-preset-delete-btn")!.click();
        },
      },
    ]);
