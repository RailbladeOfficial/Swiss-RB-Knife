...
/**
 * Makes a hex color label span double-click-editable.
 * On commit, validates the value as a 6-digit hex color, updates both the
 * color picker input and the label text, and calls onCommit with the new value.
...
function renderCombinePreview() {
  ...
  combineState.images.forEach((img, idx) => {
    ...
    // Reordering is drag-and-drop plus two arrow buttons that only appear
    // when there is somewhere to go. The menu names the same moves, and adds
    // "Make Anchor", which today means dragging a card all the way to the
    // front however long the strip is.
    attachMenu(card, () => [
      {
        label: "Make Anchor",
        disabled: idx === 0,
        onClick: () => {
          const [moved] = combineState.images.splice(idx, 1);
          combineState.images.unshift(moved);
          renderCombinePreview();
        },
      },
      {
        ...
        disabled: idx === 0,
        onClick: () => {
          swap(combineState.images, idx - 1, idx);
          renderCombinePreview();
        },
      },
      {
        ...
        onClick: () => {
          swap(combineState.images, idx, idx + 1);
          renderCombinePreview();
        },
      },
      {
        ...
        onClick: () => {
          void navigator.clipboard
            .writeText(img.path)
            .then(() => flash("Path copied", "success"))
