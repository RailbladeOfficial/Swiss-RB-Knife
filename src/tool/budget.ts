...
function buildBillRow(row: BillRow): HTMLElement {
  ...
  /* The one menu here that is about safety rather than reach. A plain click on
     an unpaid bill goes straight to Mark Paid, so every other thing you might
     want to do with the bill (look at it, edit it) currently costs you a trip
     through a payment dialog. Right-click gets at those without going near
     the payment. */
  attachMenu(el, () => [
    ...(row.kind === "paid"
      ? [{ label: "View/Edit Payment…", onClick: () => openBillAction(row) }]
      : [{ label: "Mark as Paid…", onClick: () => openBillAction(row) }]),
    { label: "Edit Bill…", onClick: () => openBillEditor(row.bill) },
    {
      label: "Copy Bill Name",
      onClick: () => {
        void navigator.clipboard
          .writeText(row.bill.name)
          .then(() => flash("Bill name copied.", "success"))
...
function makeCalBtn(
  ...
  // The extra class is how the row's right-click menu finds this button. It
  // has to go through the button rather than call a function, because the
  // date editor is an in-place swap OF this button and so needs it present.
  calBtn.className = "entry-cal-btn budget-entry-action-btn budget-entry-date-btn";
  calBtn.textContent = "📅";
...
function buildLedgerRow(item: LedgerItem): HTMLElement {
  ...
  /* The row's menu. Same two shapes Time Tracker's entry rows use: while
     Select mode is on and this row is inside a multi-row selection, the menu
     acts on the selection; otherwise it acts on this row alone.

     A bill payment row is read-only in the ledger, so its menu offers the one
     thing it can do, which is what its plain click already does. Said out
     loud in the menu rather than left as a row that appears to do nothing. */
  attachMenu(row, () => {
    if (item.kind === "bill") {
      return [{ label: "View/Edit This Payment…", onClick: () => row.click() }];
    }
    const kind = item.kind;
    const bulk = selectMode && selectedKeys.has(itemKey(item)) && selectedKeys.size > 1;
    if (bulk) {
      const list = selectedInOrder();
      return [
        { label: `${list.length} entries selected`, disabled: true },
        { label: "Duplicate All", onClick: () => cloneLedgerEntries(list) },
        { label: "Refund All", onClick: () => refundLedgerEntries(list) },
        {
          label: "Delete All",
          danger: true,
          onClick: () => openBulkDeleteModal(list),
        },
      ];
    }
    return [
      // Income's first editable column is its Source, expense's is its
      // Category, and each names a different field to makeLedgerEditable. An
      // expense additionally has its own Description; an income's name column
      // IS the source, so it has no second row here.
      ...(kind === "income"
        ? [{ label: "Edit Source", onClick: () => makeLedgerEditable(name, item, "name") }]
        : [
            {
              label: "Edit Category",
              onClick: () => makeLedgerEditable(category, item, "category"),
            },
            {
              label: "Edit Description",
              onClick: () => makeLedgerEditable(name, item, "name"),
            },
          ]),
      { label: "Edit Amount", onClick: () => makeLedgerEditable(amount, item, "amount") },
      { label: "Edit Notes", onClick: () => makeLedgerEditable(notes, item, "notes") },
      {
        label: "Edit Date",
        onClick: () =>
          actions.querySelector<HTMLButtonElement>(".budget-entry-date-btn")?.click(),
      },
      { label: "Duplicate Entry", onClick: () => cloneLedgerEntries([item]) },
      { label: "Refund Entry", onClick: () => refundLedgerEntries([item]) },
      { label: "Split Entry…", onClick: () => openSplitModal(item) },
      {
        label: "Delete Entry",
        danger: true,
        onClick: () => openDeleteModal(kind, item.entry.id),
      },
    ];
  });
