...
/** Whether the active theme has an effect playing right now, i.e. it has one
 *  AND it isn't switched off. Lets a caller tell "this theme has no animation"
 *  apart from "it has one and it's disabled" without duplicating the
 *  ANIMATED_THEMES table and the settings lookup.
 *
 *  Covers both kinds. A canvas effect registers itself in seasonalActiveTheme
 *  when it starts, so the flag alone answers for those. A CSS-driven theme has
 *  nothing to register (its motion is keyframes in its own stylesheet, running
 *  as long as the class gate lets them), so for those the answer is simply:
 *  this is the painted theme, and its animation is not switched off. */
export function isThemeEffectRunning(): boolean {
  if (seasonalActiveTheme !== null) return true;
  const painted = paintedThemeId();
  return ANIMATED_THEMES.some((t) => t.css && t.id === painted)
    && isThemeAnimationEnabled(painted);
}

/* ── Display View surfaces ────────────────────────────────────────────────
   A tool's Display View is a full-window overlay at z-index 10001 over an
   opaque fill, so everything a theme paints on the window itself is behind it
   and none of it comes through: lava's lamp, liminal's room and tape, neon's
   and terminal's scanlines. That makes the app's most theme-forward screen the
   one place the theme stops.

   These two classes are the contract that fixes it. A tool puts them on its
   overlay (and on its Setup preview, so the preview shows the same thing) and
   a theme paints onto them, without either side naming the other: adding a
   third tool with a Display View needs no theme changes, and a theme that
   paints nothing on the window needs no rule.

   The skeleton, i.e. what the two paint slots ARE, lives in landing.css:
   ::before is the ambient layer, under the card's content; ::after is the
   overlay layer, over it. A theme supplies only backgrounds and animations.
========================================================================== */

/** On the overlay whenever it is following the theme. Absent in Custom look,
 *  which is two exact colors and means them. */
const DISPLAY_SURFACE_CLASS = "theme-display-surface";
/** On the same element when the Display View's own Animations switch is off.
 *  Stills the ambient layer and drops the overlay layer; see landing.css for
 *  why the two are treated differently. */
const DISPLAY_STILL_CLASS = "theme-display-still";

/** Marks (or unmarks) an element as a themed Display View surface. Called from
 *  each tool's own applyDisplayAppearance(), so the overlay and its preview
 *  pick up a live theme change on the same signal as everything else on them. */
export function applyDisplaySurface(root: HTMLElement, themed: boolean, animate: boolean): void {
  root.classList.toggle(DISPLAY_SURFACE_CLASS, themed);
  root.classList.toggle(DISPLAY_STILL_CLASS, !animate);
}
...
/* =============================================================================
   TITLE GLOW PROBE
   -----------------------------------------------------------------------------
   Several themes put a glow on their header titles (neon's cyan bloom,
   terminal's green, Halloween's orange, Christmas's gold) and most do not.
   Anything that offers to switch that glow off has to know whether there is
   one to switch, and the honest answer is a MEASURED one: custom themes from
   the theme editor can carry a glow too, so a hard-coded per-theme table would
   be wrong the moment someone builds their own.

   #theme-glow-probe is a hidden span wearing .tool-view-title, i.e. exactly the
   rule each theme writes its title color and glow into. Reading its computed
   text-shadow answers the question for whatever theme is live, built-in or not.
   Kept in the layout tree (visibility, not display:none) so the value is real.

   Shared rather than per-tool: the TTS Repeater and the Countdown Timer both
   offer the toggle on their Display View, and two probes measuring the same
   rule is one probe too many.
============================================================================= */
 ...
 *
 *  `css: true` marks the entries with no canvas branch below: their motion is
 *  keyframes living in the theme's own stylesheet, gated on the body class
 *  that applyThemeAnimationClass() maintains. They are listed here so they
 *  share one toggle list with the canvas effects rather than growing a second
 *  one, and the flag is what lets isThemeEffectRunning() answer for them
 *  without a second hard-coded list of names. */
export const ANIMATED_THEMES: { id: string; label: string; effect: string; css?: true }[] = [
  ...
  { id: "lava", label: "Lava", effect: "Drifting lava blobs", css: true },
  { id: "liminal", label: "Liminal", effect: "Drifting haze and tape tracking", css: true },
];
