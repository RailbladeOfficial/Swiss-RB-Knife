...
async function fetchThemePreviewVars(themeId: string): Promise<Record<string, string>> {
  ...
  try {
    ...
    // Preview tile just keeps its CSS-default colors if the fetch fails.
  }
...
function buildCustomThemeTile(theme: CustomTheme): HTMLElement {
  ...
  // The same three actions the tile already offers, without having to hit one
  // of the two small icon buttons in its footer.
  attachMenu(tile, () => [
    { label: "Apply", onClick: () => selectCustomTheme(theme.id) },
    {
      label: "Edit Theme…",
      onClick: () => {
        themePickerModal.close();
    ...
    },
    {
      label: "Delete Theme",
      danger: true,
      onClick: () => requestDeleteCustomTheme(theme.id),
    },
  ]);
  return tile;
}
