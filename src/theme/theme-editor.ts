...
/** Best-effort conversion of a CSS color value to a 6-digit hex string.
...
export function openThemeEditor(mode: "create" | "edit", id?: string): void {
  ...
  if (mode === "create") {
    ...
    teNameInput.value = "";
    // Seed from whatever's currently rendered (the active theme's colors).
    ...
    teBaseCurrentOption.textContent = "This theme's saved colors";
    tePopulateSwatches(_teWorkingVars);
...
document.querySelectorAll<HTMLElement>(".te-row[data-var]").forEach((row) => {
  ...
  // Swatch moved by color picker: update vars + preview, and update the hex
